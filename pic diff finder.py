...
def compare(i1, i2, diff_percent=10, diff_color=(255, 0, 0, 255)):
    def cls(inp):
        return [inp.getpixel((width, height)) for height in range(inp.size[1]) for width in range(inp.size[0])]
    ...
    mn = min(color_diff)
    mx = max(color_diff) - mn
    color_diff_bool = [(pixel - mn) / mx * 100 >= diff_percent for pixel in color_diff]
